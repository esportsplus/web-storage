...
class WebStorageDriver<T> implements Driver<T> {

    private prefix: string;
    private storage: Storage;


    constructor(storage: Storage, name: string, version: number) {
    ...
    async all(): Promise<T> {
        ...
        for (let i = 0, n = keys.length; i < n; i++) {
            let value = this.parse(this.storage.getItem(this.prefix + keys[i]));
    ...
    async clear(): Promise<void> {
        ...
        for (let i = 0, n = keys.length; i < n; i++) {
            this.storage.removeItem(this.prefix + keys[i]);
        }
    }
    ...
    async only(keys: (keyof T)[]): Promise<Map<keyof T, T[keyof T]>> {
        ...
        for (let i = 0, n = keys.length; i < n; i++) {
            let value = this.parse(this.storage.getItem(this.key(keys[i])));
    ...
    async replace(entries: [keyof T, T[keyof T]][]): Promise<void> {
        for (let i = 0, n = entries.length; i < n; i++) {
            this.storage.setItem(this.key(entries[i][0]), this.serialize(entries[i][1]));
        }
    }

    async set(key: keyof T, value: T[keyof T]): Promise<boolean> {
        try {
            this.storage.setItem(this.key(key), this.serialize(value));
            return true;
        }
        catch {
            return false;
